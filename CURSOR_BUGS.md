# Cursor, Focus, and Overlay Bugs — Findings & Fixes

This document records recent UI bugs in `react-notepad` (v3 branch) involving cursor placement, tapping, focus, and overlay alignment. It also notes which issues are specific to the Markdown overlay editor vs the plain text editors.
...
- Added `onTouchEnd` with `requestAnimationFrame` as a mobile fallback.
...
### Fix
Added `overflow: hidden` to the `OverlayLine` divs in `MarkdownOverlayNotepad`. This clips the scaled heading glyphs to their own row, preventing visual overflow into adjacent rows. The scaled-heading feature itself is preserved.
